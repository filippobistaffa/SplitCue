...
static char *split_point_file = TEMP_CUE;
...
bool set_metadata(char *filename, char *artist, char *album, char *genre, char *year, char *song, char *number, struct cover_struct chunk)
{
	...
	FLAC__metadata_simple_iterator_init(iter, filename, FALSE, FALSE);	
	FLAC__StreamMetadata_VorbisComment_Entry entry;
	
	char *strings[TAG_NUMBER] = {artist, album, year, genre, song, number};
	char *tags[TAG_NUMBER] = {"ARTIST", "ALBUM", "DATE", "GENRE", "TITLE", "TRACKNUMBER"};
	int i;	
	
	for (i = 0; i < TAG_NUMBER; i++)
	{
		entry.length = strlen(tags[i]) + strlen(strings[i]) + 1; // "="
		entry.entry = malloc(entry.length + 1); // \0
		sprintf((char *)entry.entry, "%s=%s", tags[i], strings[i]);		
...
static bool split_file(wave_info *info)
{
  ...
  char path[FILENAME_SIZE], outfilename[FILENAME_SIZE];
  int current;
  wint discard,bytes;
  bool success;
  ...
  char *title = (char *)gtk_entry_get_text(GTK_ENTRY(data->widgets->title_entry));
  char *genre = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(data->widgets->combobox));
  sprintf(year, "%i", gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(data->widgets->spin_button)));  
  sprintf(path, "%s/%s", DESTINATION, artist);
  mkdir(path, S_IRWXU);
  sprintf(path, "%s/%s - %s - %s", path, artist, year, title);
  mkdir(path, S_IRWXU);
  ...
  for (current=0;current<numfiles;current++) {
    ...
    */
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(data->album->songs), &iter, 1, &song, -1);
    sprintf(n, "%02d", gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(data->widgets->offset)) + current + 1);
    sprintf(outfilename, "%s/%s - %s - %s.%s", path, artist, n, song, FORMAT);
    gtk_tree_model_iter_next(GTK_TREE_MODEL(data->album->songs), &iter);
    ...
    if (0 != current) {
      ...
      if (transfer_n_bytes2(info->input,files[current]->output,
      			    files[current-1]->output,leadin_bytes+leadout_bytes,
